...
第 4 阶段支持 Redis mode，用于打通 `frontend-vue → backend-java → Redis → inference-python worker → Redis → backend-java SSE → frontend-vue`。
...
- `queue:mmvs:tasks`：待处理任务队列。
- `task:{taskId}`：任务状态、视频路径、用户指令和时间戳。
- `stream:task:{taskId}`：状态、阶段、Token 指标、摘要片段、完成或错误事件。
- `result:{taskId}`：最终摘要、Token 指标和耗时 JSON。

模式说明：

- `APP_MODE=in-memory`：Java 后端本地 mock 调度器继续可用，不依赖 Redis。
