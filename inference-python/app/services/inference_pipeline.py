...
class InferencePipeline:
    ...
    def run(self, task_id: str, video_path: str, query_text: str) -> InferenceResult:
        ...
        self.stream_publisher.publish(task_id, "stage", {"stage": "video_preprocess", "sampled": sampled_frames.sampled_count}, "video_preprocess")

        started = perf_counter()
        patch_features = self.encoder.encode_mock(sampled_frames)
        collector.set_stage("encode_ms", elapsed_ms(started))
        self.stream_publisher.publish(task_id, "stage", {"stage": "video_swin", "shape": list(patch_features.shape)}, "video_swin")
        ...
        self.stream_publisher.publish(task_id, "token_metrics", token_metrics.model_dump(), "token_compression")

        started = perf_counter()
        projected_tokens = self.adapter.project_mock(compressed_tokens)
        collector.set_stage("project_ms", elapsed_ms(started))
        self.stream_publisher.publish(task_id, "stage", {"stage": "projection_adapter", "shape": list(projected_tokens.shape)}, "projection_adapter")

        started = perf_counter()
        summary_chunks: List[str] = []
        for chunk in self.generator.stream_generate(metadata, query_text, token_metrics):
            summary_chunks.append(chunk)
            self.stream_publisher.publish(task_id, "summary_delta", {"text": chunk}, "summary_generation")
        ...
        self.stream_publisher.publish(task_id, "completed", result.model_dump(), "finished")
        self.stream_publisher.save_result(task_id, result.model_dump())
        return result
